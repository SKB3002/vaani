...
- **`app/services/insights/aggregator.py`** — `build_monthly_bundle(month, ledger) -> MonthlyStatsBundle`. Pandas reads expenses/budgets/wishlist/investments and produces a typed bundle with current month, previous month, trailing-3m, trailing-12m windows. `bundle_hash(bundle)` is deterministic (excludes `generated_at`). The `goals` field of the bundle is populated from the wishlist table (the standalone Goals feature was removed; wishlist items are the savings goals).
...
- **`app/services/insights/cache.py`** — `InsightsCache` (CSV + Supabase dual-mode via `LedgerWriter`). Cache key composes `(kind, bundle_hash, month, prompt_version, model)` so prompt-template revisions force regeneration. `make_invalidator(cache)` is registered on `LedgerWriter.on_change` in `app/main.py` lifespan; mutations to `expenses`/`budget_rules`/`budget_table_c`/`wishlist`/`investments` invalidate; other tables are no-ops to avoid invalidation noise.
